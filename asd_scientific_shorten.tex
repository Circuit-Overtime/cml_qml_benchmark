...
\end{center}
\abstract*{
This research investigates the application of both classical and Quantum Machine Learning (QML) strategies for predicting Autism Spectrum Disorder (ASD) traits. ASD is a neuro-developmental condition impacting communication, cognition, and social abilities, where early diagnosis is crucial for improved outcomes. Various supervised learning techniques, including Logistic Regression, Decision Trees, K-Nearest Neighbors, Naive Bayes, Random Forest, Support Vector Machine (SVM), Histogram Gradient Boosting Classifier (HGBC), as well as Quantum Support Vector Machine (QSVM) and Variational Quantum Classifier (VQC) were explored on both full and PCA-reduced datasets. 

Among classical models, HGBC achieved the highest accuracy (97.86\%), precision (98.48\%), recall (97.50\%), and F1-score (97.90\%), demonstrating its robust capability in identifying ASD traits while minimizing false positives. Quantum models showed potential, with QSVM achieving high accuracy in benchmark studies (98.9\%), though our experiments were limited by smaller circuit depth, fewer epochs, and dataset size. These results suggest QML approaches may offer advantages for high-dimensional or overlapping datasets in the future. 

Exploratory Data Analysis revealed that ASD traits are more prevalent in children, particularly males, and highlighted potential influences of ethnicity and family history. Overall, this study demonstrates that classical ensemble methods remain highly effective for ASD prediction, while QML approaches provide promising avenues for future enhancement. The findings support the integration of computational methods into early ASD screening programs to improve timely intervention and clinical outcomes.
} \\
\textbf{Keywords:}
\keywords*{Autism Spectrum Disorder (ASD), Quantum Machine Learning, Classification Algorithms, Logistic Regression, KNN, SVM, HGBC, QSVM, Variational Quantum Classifier}
